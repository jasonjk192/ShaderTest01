Shader "ShaderTest/VolumetricCloudShader"
{
    Properties
    {
        _Scale("Range", Range(0.1, 10.0)) = 2.0
        _StepScale("Step Scale", Range(0.1, 100.0)) = 1.0
        _Steps("Steps", Range(1,200)) = 64
        _MinHeight("Min Height", Range(0.0,5.0)) = 0.0
        _MaxHeight("Max Height", Range(6.0,10.0)) = 10
        _FadeDistance("Fade Distance", Range(0.0,10.0)) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float random(float3 value, float3 dotDir)
            {
                ...
                random = frac(sin(random) * 123456.789);
                return random;
            }

            float3 random3d(float3 value)
            {
                return float3(  random(value, float3(1.2,2.3,3.4)),
                                random(value, float3(2.2,3.3,4.4)),
                                random(value, float3(3.2,4.3,5.4)));
            }
            ...
            // #define allows to pass functions as arguments
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float depth = 1;
                depth *= length(i.viewDir); // Can use Camera Depth Texture instead too
